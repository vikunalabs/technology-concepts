...
### 3.4 A custom, more explicit JobFactory
...
### 3.5 The critical Step: Actually wiring the factory in (the part everyone forgets)
...
Spring Boot's autoconfiguration **auto-detects `JobDetail` beans and `Trigger` beans** in your context. That part is real and documented. **But `JobFactory` is not on that auto-detected list.** Instead, Spring Boot's own `quartzScheduler()` method constructs its own plain `SpringBeanJobFactory` right there — **unconditionally** — before any customizer beans run.
...
2. **Explicitly wire a Spring-aware `JobFactory`** — either rely on Spring Boot's default `SpringBeanJobFactory` (which works for most cases) or provide a custom one via `SchedulerFactoryBeanCustomizer`.
...
### 3.9 Common failure modes and their causes
