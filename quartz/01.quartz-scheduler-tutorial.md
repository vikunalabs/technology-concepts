...
> **Important:** If your job class uses constructor injection and you're using
> `JobBuilder.newJob(MyJobClass.class)`, the job class **must** be annotated with
> `@Component` AND you need `SpringBeanJobFactory` (which Spring Boot provides
> by default). Without the annotation, SpringBeanJobFactory will try to
> instantiate it via no-arg constructor and fail.
...
Writing a custom `JobFactory` class is necessary but **not sufficient**. Quartz's `Scheduler` has to be told to use it. In a pure Java environment, this is straightforward:
...
It is never automatic. The combination of `@Component` on the job class plus a properly wired `JobFactory` is what makes constructor injection work.
